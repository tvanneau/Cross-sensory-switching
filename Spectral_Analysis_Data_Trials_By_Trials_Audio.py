...
for ix in range(0,len(list_files_epochs),1):
    ...
    epochs_stim.set_eeg_reference(['E17','B18'])
    
    # Define the clusters of electrodes
    
    # Occipital visual cluster
    cluster_1 = ['D1','C1','D15','D2','C25']
    ...
    columns = ['ERP_MinPeak_70_150ms_FCz','ERP_MinPeakLatency_70_150ms_FCz',
               'ERP_MaxPeak_160_220ms_FCz','ERP_MaxPeakLatency_160_220ms_FCz']
    ...
    for idx in range(len(epochs_stim)):
        ...
        data_1_70_150ms = epoch.copy().crop(tmin=0.07, tmax=0.15).pick_channels(cluster_1).get_data().mean(axis=1)
        max_peak_1 = data_1_70_150ms.min()
        max_peak_latency_1 = (data_1_70_150ms.argmin() / sfreq) + 0.07
        stim_df.loc[idx, 'ERP_MinPeak_70_150ms_FCz'] = max_peak_1
        stim_df.loc[idx, 'ERP_MinPeakLatency_70_150ms_FCz'] = max_peak_latency_1
        
        # Extract the minimum peak and latency for the specified window and cluster
        data_1_160_220ms = epoch.copy().crop(tmin=0.16, tmax=0.22).pick_channels(cluster_1).get_data().mean(axis=1)
        min_peak_1 = data_1_160_220ms.max()
        min_peak_latency_1 = (data_1_160_220ms.argmax() / sfreq) + 0.16
        stim_df.loc[idx, 'ERP_MaxPeak_160_220ms_FCz'] = min_peak_1
        stim_df.loc[idx, 'ERP_MaxPeakLatency_160_220ms_FCz'] = min_peak_latency_1
    ...
    # # For trial by trial normalisation using the common baseline from all trials
    # tmin_baseline = -0.3
    # tmax_baseline = -0.05
    # M_baseline = power.average()
    # M_baseline_NPL = power_NPL.average()
    
    
    # Baseline = M_baseline.crop(tmin=tmin_baseline,tmax=tmax_baseline).data
    # Baseline_time = np.mean(Baseline, axis=2)
    # Baseline_PL = Baseline_time[:,:,np.newaxis]
    # normalized_tfr = 10*np.log10(power.data / Baseline_PL)
    # power.data = normalized_tfr
    
    
    # Baseline_NPL = M_baseline_NPL.crop(tmin=tmin_baseline,tmax=tmax_baseline).data
    # Baseline_time_NPL = np.mean(Baseline_NPL, axis=2)
    # Baseline_NPL = Baseline_time_NPL[:,:,np.newaxis]
    # normalized_tfr_NPL = 10*np.log10(power_NPL.data / Baseline_NPL)
    # power_NPL.data = normalized_tfr_NPL
    
    # del M_baseline, M_baseline_NPL, normalized_tfr, normalized_tfr_NPL
    # gc.collect()
    ...
    stim_df.to_csv('Data_trials_Audio_Subject_'+Subject+'.csv', index=False)

    with open('Power_Audio_Preceded_Visual_Subject_'+Subject+'.pkl', 'wb') as fp:
        pickle.dump([tfr_v_avg, tfr_NPL_v_avg], fp)    
    with open('Power_Audio_Preceded_Audio_Subject_'+Subject+'.pkl', 'wb') as fp:
